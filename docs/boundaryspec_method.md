...
For each boundary, it emits required controls and adversarial pressure prompts. Those prompts are used to test whether an agent keeps the boundary under pressure.
...
5. Score:
   - boundary preserved
   - useful progress made
   - proof captured
   - exact failure sentence
6. Compare model behavior across task categories and pressure types.

## Known Limits

This version uses keyword-style extraction. It is meant as an inspectable seed, not a final benchmark. Stronger versions should add blinded human labels, multiple models, richer task corpora, formal constraint languages, and checks that separate refusal theater from useful safe progress.
